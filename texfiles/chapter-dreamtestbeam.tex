...
The initial approach to calibrate the towers was to make a histogram of the \acrshort{ADC}s of the scintillators of all events in the tower under the beamspot, for each of the 29 pion runs, and determine the mean of each channel. A calibration coefficient was then found:
...
The analysis module was then re-run, multiplying all ADC values by the calibration coefficient for each tower. The results can be seen in Fig. [ref]. This did not result in an even calibration of all \acrshort{ADC}s and it was proposed that this was due to the beam composition, as this calibration procedure was done without any particle selection.

Because particles were not selected, the calibration here does not account for the different ways pions and muons deposit energy in the calorimeter. As pions are hadrons, they will produce hadronic showers, spreading their energy within the shower radius for a certain depth, then stop. Muons instead act as minimum ionising particles (\acrshort{MIP}s), depositing energy along their path through through the calorimeter ionisation losses.

In order to address this, calibration was done twice more, using particle selection criteria described in Section \ref{section:particle-selection}.
...
The same procedure as in Section \ref{section:tower-calibration} was then used, using only muons.
...
\centering
	...
	\hline \hline
	\textbf{Tower} & \textbf{Run No.} & $C_{all}$ & $C_{\mu}$ & $C_{\pi}$ \\ \hline
	 1 & 12545 & 0.4551 & x & x \\
	 2 & 12556 & 0.4450 & x & x \\
	 3 & 12558 & 0.5842 & x & x \\
	 4 & 12560 & 0.5008 & x & x \\
	...
	27 & 12607 & 0.4738 & x & x \\
	28 & 12604 & 0.5690 & x & x \\
	29 & 12602 & 0.4405 & x & x \\ \hline \hline
	\end{tabular}
	\caption{Table of towers in the RD52 calorimeter with the run number of their calibration run, and calibration coefficients for all particles ($C_{all}$), muons ($C_{\mu}$), and pions ($C_{\pi}$).}
	\label{table:idea/calibrationruns}
\end{table}
...
To calibrate for energy, the total \acrshort{ADC} for each event was found by summing the invidual ADC of each channel in the event after the pedestal subtraction. Three of these sums were done -- one of only the scintillator channels, one of only the Cherenkov channels, and one of both sets of channels. The resulted in three totals for each event.

Three histograms were then made, filled with each one of the three totals, producing a spectrum of total \acrshort{ADC} count for all scintillator channels (Fig. \ref{figure:testbeam/results/total-adc-sc}), and all Cherenkov channels (Fig. \ref{figure:testbeam/results/total-adc-ch}), and both combined (Fig. \ref{figure:testbeam/results/total-adc-both}).

The data was then fitted with a gaussian using ROOT to determine the mean and standard deviation, and the value of the mean \acrshort{ADC} taken to be the \acrshort{ADC} value for a 20 GeV event. This means that for a 20 GeV event, the total \acrshort{ADC} count will be [x], the scintillator \acrshort{ADC} count will be [x], and the Cherenkov \acrshort{ADC} count will be [x].

This can then be checked by examining Run 12512, which is an 80 GeV run using a secondary beam of predominantly pions, with some muons. Taking the total \acrshort{ADC} count of these runs as [x], this gives an energy of [x]. [...]
...
\begin{figure}[p]
	...
	\label{figure:testbeam/results/total-adc-both}
\end{figure}
