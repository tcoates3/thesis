...
% Nigel: Please add here or earlier, a few short paragraphs, at level of one/package, to illustrate alternative solutions that already exist and comment on why these are not generic in the sense that you need them to be for test beam use. Could look at what has been done e.g. at LHC or other large projects? Cursory treatment, but I think this would be a useful addition, also shows that DQM4hep is not an example of 'reinventing the wheel', which is best justified if you have checked for the existence of wheels yourselves.

(EUDAQ monitor, which is specific to beam telescopes at a hard-coded level)
