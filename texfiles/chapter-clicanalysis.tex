...
The initial step of the analysis procedure is to distinguish between the hadronic and semi-leptonic final states. This is done by finding isolated leptons in the final state. If there are no final-state leptons it is classified as a hadronic event; if there is a single lepton in the final state it is classified as a semi-leptonic event. If there are two leptons in the final state it is classified as a leptonic event and discarded.

In the fully hadronic decay channel, there are eight jets in the final state, so any 

[...]

There is another contribution to the tth final state in the higgstrahlung process. This process is actually a $e^+e^- \rightarrow t\overline{t} $ event with the intermediate $Z/\gamma *$ radiating a Higgs boson before decaying into the $t\overline{t}$ pair. However, the final state is $tth$, but without any direct interaction between the top quark and Higgs (see Fig. \ref{figure:physics/SM/higgstrahlung}). This means that the higgstrahlung process is not sensitive to the top-Higgs Yukawa coupling, and these events must be taken into account when calculating the total cross-section of the tth process.
