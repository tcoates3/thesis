...
\begin{figure}[h]%
	\centering
    \subfloat{{\includegraphics[width=0.45\textwidth]{../Pictures/SiDCutaway.jpg} }}%
    \qquad
	\subfloat{{\includegraphics[width=0.45\textwidth]{../Pictures/SiDQuadrant.png} }}%
...
As of writing, the \acrshort{CEPC} project released a \acrfull{CDR} in 2018 detailing the current state of planning for the experiment. It has also been submitted input to the European Particle Physics Strategy Update. The current timeline envisions a five-year R&D period from 2018-2022, and construction to start in 2022, completing in 2030. The physics programme is expected to last for ten years, concluding in 2040. 
...
The detectors of the \acrshort{CEPC} as outlined in the \acrshort{CDR} are very similar in design to the \acrshort{ILD}, in the design of using a silicon vertex tracker, a TPC tracker, and similar options for the calorimeter. An additional difference is the optioning of a dual-readout calorimeter to replace the electromagnetic and hadronic calorimeters. However, there would be two interaction points around the ring, allowing both detectors to operate at the same time, as opposed to the \acrshort{ILC}'s push-pull system. More information on the proposed \acrshort{CEPC} detectors can be found in [reference].